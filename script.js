...
const API_FORECAST_URL = "https://api.open-meteo.com/v1/forecast?latitude=52.52&longitude=13.41&hourly=temperature_2m,relative_humidity_2m,weather_code,wind_speed_10m";
    ...
    if (navigator.geolocation) {
        navigator.geolocation.getCurrentPosition(async position => {
            ...
            try {
                ...
                displayWeather("orasul tau", weatherData);
...
async function getCityCoordinates(cityName) {
    ...
    let selectedCity = data.results.find(city => city.timezone === "Europe/Bucharest") || data.results[0];

    return { lat: selectedCity.latitude, long: selectedCity.longitude };
}
...
async function getWeather(lat, long) {
    ...
    if (Array.isArray(data)) {
        for (let obj of data) {
            if (obj.latitude === lat && obj.longitude === long) {
                return obj;
            }
        }
    }
